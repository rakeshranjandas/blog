---
draft: false
tags:
  - copywork
date: 2025-09-02
---
 
# How To Tell a Great Story

**Author** - Seth Godin
**Source** - https://seths.blog/2006/04/ode_how_to_tell/


### Summary

**Structure of the post**
Great Stories capture
- are consistent and authentic
- make a bold promise
- trusted
- subtle
- first impression
- not logic but senses
- aimed not at average, but a tiny niche audience
- don't have contradictions
- makes the audience feel smart


The author speaks on about how great stories captivate. They capture the senses of the audience by empathetic storytelling and being bold. It is important for them to come out of credible sources so that it is easy for people to trust on them. They are consistent. They are authentic to the point of compromising a larger audience for a niche set of interested people. Great stories make people feel safe and smart.

### How I felt
- This post was well organised with short paragraphs. I attempted to memorise one paragraph at a time.
- Reading as if you are giving speech certainly helps grabbing the emotions of the lines.
- Often when I tried to write after memorising, I did not remember the exact phrases and when I made them out with my default style, I learned that there are lot of redundancy in my framing of sentences. e.g.
	-`to be able to` can be removed entirely
	-I wrote`the most effective stories are those that match the worldview`. It is actually simple in the post- `the most effective stories match the worldview`. 
	-should use `the` more instead of `that`. e.g. `spread that story` -> `spread the story`. "the" means implicit already, no need to be more specific using "that".